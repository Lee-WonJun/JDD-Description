...
- 오류가 나면 무시해라, 누가 뭐라고 하면 Ostrich Algorithm 이라고 
...
- N+1 문제를 자동으로 해결되지 않는 이 세상이 이상한거다. 개발자는 신경쓰지 말자
- fetch join + paging 문제를 자동으로 해결되지 않는 이 세상이 이상한거다. 개발자는 신경쓰지 말자
- slow query 문제를 자동으로 해결되지 않는 이 세상이 이상한거다. 개발자는 신경쓰지 말자
