...
- 커밋은 나눌 때는 작업이나 기능 단위가 아니라 내가 한 번에 고친만큼을 기준으로 한다. 어차피 머지되면 다 Squash 되서 잘 안보인다.
...
- 그건 프론트엔드가 해야 할 일이라고 해라.
  - 그건 원래 백앤드 팀의 일이 어딜가든 프론트엔드보다 항상 많아서 그런 것이라고 해라.
...
- 그건 백엔드가 해야 할 일이라고 해라.
  - 그건 원래 백앤드 팀에서 개발이 안 되면 프론트엔드에서는 할 일이 없다고 해라.
...
- 그건 DBMS 개발자가 PL/SQL 으로 비즈니스 로직을 구현해야할 일이라고 해라.
```
...
- 간단한 기능만 짜놓고 열심히 하는 척 게으름 피우자. 누가 뭐라고 하면 요새 유행하는 '린Lean'하게 개발하는 방법론이라고 말해라
```
