...
- 동료들을 성장시키려면 물고기를 떠먹여주지 말고, 낚시하는 방법을 가르쳐줘라.
함수 입력값에 대한 반환 결과를 기대하는 핑거 프린세스들에게 람다함수로 반환해서 그 함수를 직접 경험하게 하자.
처음에 시작할 땐 어렵고 복잡해보이는 코드도 람다라는 단순한 이름으로 사용할 수 있다는 교훈을 알려줘야 한다.
```
...
- 테스트가 어떨때는 성공하고 어떨 때는 실패하면, "속성 기반 테스트" 를 작성해서 그렇다고 해라
...
- 성능최적화는 그 어떠한 경우에도 금물, 누가 뭐라고 하면 요즘은 "컴파일러" 한테 맡기는 게 대세라고 해라
...
- 위에서 결과물을 가지고 쪼으면 개발자들이 퍼포먼스가 안 나온다고 말해라.
...
- 그 외 언젠가 한번쯤 읽어본 책들 다수
- 그 외 언젠가 한번쯤 읽어볼 책들 다수
- 나무위키
