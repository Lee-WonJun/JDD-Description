...
```
- PR 은 항상 LGTM(Looks Good To Me)을 명심하자. 누군가 PR 을 했다는 사실 자체가 보기 좋은 일이다. 
...
```
- 공격이 들어오면 코드로 방어하지 말고 너에게 들어오는 일감을 방어해라

- 작업양이 많은가? 그냥 특정 테스트 케이스에서만 돌아가게 짜고 나머지는 "고도화" 작업에서 한다고 해라. 그리고 "고도화" 작업 하기전에 이직을 해라.
```
...
- 커밋은 나눌 때는 작업이나 기능 단위가 아니라 내가 한 번에 고친만큼을 기준으로 한다. 어차피 머지 되면 다 Squash 되서 잘 안보인다.
...
- 리스트 컴프리헨션, 옵셔널 체이닝, 리엑티브 등을 쓰고 자랑스럽게 "모나딕하게 해결했다"고 말해라.
...
- 간단한 기능만 짜놓고 열심히 하는 척 게으름 피우자. 누가 뭐라고 하면 '린Lean'하게 했다고 하자
```
